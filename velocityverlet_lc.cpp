...
void VelocityVerlet_LC::timestep(real delta_t)
{
  ...
  // ask our thermostat if necessary
  if (W_LC.thermostat_target_temperature != 0) 
    {
      thermostat();
    }
...
// thermostat adjusts the system be modifying the velocities
void VelocityVerlet_LC::thermostat()
{
  // initialize helpers
  unsigned counter = 0;
  
  // set counter
  counter = (unsigned)(W_LC.t/W_LC.delta_t);

  // initialize the resclaling coeffizient
  real beta = 0;


  // exit condition
  if ((counter%(W_LC.thermostat_step_interval)) != 0) return;

  // else rescale velocities
  else
    {
      // set rescaling coeffizient
      beta = ( W_LC.thermostat_target_temperature *( W_LC.particles_N -1.0 )) /( 48.0 *  W_LC.e_kin);
      beta = sqrt( beta );

      // go through every cell and every particle
      // initialize iterator for cells
